...
module.exports = {
    createUser: async function ({ userInput }, req) {
        ...
        if(errors.length > 0){
            const error = new Error('invaild input');
            error.data = errors;
            error.code = 422;
            throw error;
        }
        ...
        console.log(userInput.email, userInput.name, userInput.password, hashedPw)
        const user = new User({
    ...
    createPost: async function ({ postInput }, req) {
        ...
        || !validator.isLength(postInput.title, { min: 5})){
            errors.push({ message: 'Title Invaild.'})
        }
        if(validator.isEmpty(postInput.content) 
        || !validator.isLength(postInput.content, { min: 5})){
            errors.push({ message: 'Content Invaild.'})
        }
        if(errors.length > 0){
            const error = new Error('invaild input');
            error.data = errors;
